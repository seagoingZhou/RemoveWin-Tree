...
def _reset_redis(sshs):
    print("scp writing")
    for ssh in sshs:
        stdin, stdout, stderr = ssh.exec_command("rm -rf ~/RemoveWin-Tree")
        time.sleep(1)
        s = scp.SCPClient(ssh.get_transport())
        s.put("../../RemoveWin-Tree", remote_path="~/", recursive=True)
        s.close()
        stdin, stdout, stderr = ssh.exec_command("cd ~/RemoveWin-Tree/redis-4.0.8/;sudo make install", get_pty=True)
        stdin.write("user\n")
        stdin.flush()
        data = stdout.read()
        if len(data) > 0:
